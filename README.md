...
我们的实现分为4步:
\begin{itemize}
  \item[1)] 均匀划分+局部快速排序+正则采样
  \item[2)] 归并排序正则采样数据+正则采样
  \item[3)] 数据分区+分区合并
  \item[4)] 归并排序
\end{itemize}

\subsubsection{Step1:均匀划分+局部快速排序+正则采样}
...
\subsubsection{Step2:归并排序正则采样数据+正则采样}
...
int *sdispls=new int[nprocs];//表示发送缓冲区中每个数据块的
                             //偏移量,第i个元素表示当前进程
                             //要发送给第i个进程的数据块在
                             //发送缓冲区中的起始位置
sdispls[0]=0; 
...
// 分区合并
MPI_Alltoallv(myArr,sendcounts,sdispls,MPI_INT,*myArr2,
              recvcounts,rdispls,MPI_INT,MPI_COMM_WORLD);
```


\subsubsection{Step4:归并排序}
...
// 收集排序好的数组到根进程
int* recvbuf=new int[nprocs];  //指向根进程的缓冲区的指针,
                               //存放所有进程发送的子列表大小
...
MPI_Gatherv(mySortedArr2,mySize2,MPI_INT,arr,recvbuf,rdispls,
            MPI_INT,0,MPI_COMM_WORLD);
```
...
我们测试了程序在1K、5K、10K、100K、1M、10M等不同数据量以及在2、4、8、16个线程数情况下的加速比，实验结果如下。
\begin{table}[h]
  \caption{并行程序在1K、5K、10K、100K、1M、10M等不同数据量以及在2、4、8、16个线程数情况下的加速比}
  \centering
  \begin{tabular}{lcccccc}
  \toprule
  & 1K & 5K & 10K & 100K & 1M & 10M \\
  \midrule
  2线程 & \textbf{0.76} & 1.33 & 1.41 & 1.35 & 1.60 & 1.87 \\
  4线程 & 0.47 & \textbf{1.64} & \textbf{2.17} & 2.03 & 2.88 & 3.41 \\
  8线程 & 0.15 & 0.70 & 1.26 & \textbf{2.50} & 3.81 & 4.67 \\
  16线程& 0.05 & 0.28 & 0.38 & 1.61 & \textbf{3.85} & \textbf{5.00} \\
  \bottomrule
  \end{tabular}
  \label{table:perf2}
  \end{table}
  ...
  从以上结果，我们可以得到如下结论：
  \begin{itemize}
    \item[1.] 在实验测试的数据量下，固定线程数，加速比随着数据量的增加而增加，这说明并行程序的额外开销不可忽视。
    \item[2.] 在数据量较小的时候，串行算法本身就已经足够快速，而并行算法额外的启动以及通信等一系列开销使得并行算法比串行算法还慢。这一问题随着线程数增加更为明显。
    \item[3.] 并行算法最大加速比对应的线程数随着数据量的增加而增加。这说明了增大数据量才能更好地抵消更多线程所带来的额外开销。
    \item[4.] 在数据量较大的时候，并行算法展现出了非常大的优势，在16线程、10M数据量的情况下，加速比甚至达到了5。此外，PSRS算法的效率也很高，在2/4线程、10M数据量下，加速比十分接近于线程数，这说明了选取合适主元的重要性。
  \end{itemize}
